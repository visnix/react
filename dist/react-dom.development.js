...
function updateNamedCousins(rootNode, props) {
  ...
  if (props.type === "radio" && name != null) {
    ...
    for (let i = 0; i < group.length; i++) {
      ...
      invariant(); // We need update the tracked value on the named cousin since the value
...
function getHostProps$3(element, props) {
  const node = element;
  invariant(props.dangerouslySetInnerHTML == null); // Always set children to the same thing. In IE9, the selection range will
...
const PerformedWork =
  /*                */
  0b000000000000000000001; // You can change the rest (and add more).
...
const HostEffectMask =
  /*               */
  0b000000001111111111111; // These are not really side effects, but we still reuse this field.
...
const PassiveStatic =
  /*                */
  0b001000000000000000000; // These flags allow us to traverse to fibers that have effects on mount
// don't contain effects, by checking subtreeFlags.

const BeforeMutationMask = // TODO: Remove Update flag from before mutation phase by re-landing Visiblity
...
const MutationMask =
  ...
  Hydrating |
  Visibility;
const LayoutMask = Update | Callback | Ref; // TODO: Split into PassiveMountMask and PassiveUnmountMask
...
function assertIsMounted(fiber) {
  invariant(getNearestMountedFiber(fiber) === fiber);
}
...
function findCurrentFiberUsingSlowPath(fiber) {
  ...
  while (true) {
    ...
    invariant(a.alternate === b);
  } // If the root is not a host container, we're in a disconnected tree. I.e.
  // unmounted.

  invariant(a.tag === HostRoot);
...
const {
  ...
  unstable_wrap
} = ReactInternals$1.SchedulerTracing;
...

{
  ...
  // scheduler/tracing
  invariant(__interactionsRef != null && __interactionsRef.current != null);
}
...
const UIEventInterface = { ...EventInterface, view: 0, detail: 0 };
...
const DragEventInterface = { ...MouseEventInterface, dataTransfer: 0 };
...
const FocusEventInterface = { ...UIEventInterface, relatedTarget: 0 };
...
const CompositionEventInterface = { ...EventInterface, data: 0 };
...
function getOffsets(outerNode) {
  ...
  const { anchorNode, anchorOffset, focusNode, focusOffset } = selection; // In Firefox, anchorNode and focusNode can be "anonymous divs", e.g. the
...
const nonDelegatedEvents = new Set([
  ...
  "load",
  "scroll",
  "toggle", // In order to reduce bytes, we insert the above array of media events
...
function accumulateSinglePhaseListeners(
  ...
  while (instance !== null) {
    const { stateNode, tag } = instance; // Handle listeners that are on HostComponents (i.e. <div>)
...
function findCurrentUnmaskedContext(fiber) {
  {
    ...
    invariant(isFiberMounted(fiber) && fiber.tag === ClassComponent);
    let node = fiber;
...
const getLabelsForLanes$1 = getLabelsForLanes;
/**
...
function formatLanes(laneOrLanes) {
  let labels = getLabelsForLanes$1(laneOrLanes);
...
function markComponentSuspended(fiber, wakeable) {
  {
    if (supportsUserTimingV3) {
      ...
      const componentName = getComponentNameFromFiber(fiber) || "Unknown"; // TODO Add component stack id
...
function readContext(context) {
  ...
  if (lastFullyObservedContext === context);
    ...
    if (lastContextDependency === null) {
      invariant(); // This is the first dependency for this component. Create a new list.
...

{
  ...
  warnForMissingKey = (child, returnFiber) => {
    ...
    invariant(typeof child._store === "object");
...
function coerceRef(returnFiber, current, element) {
  ...
  if (
    ...
    if (element._owner) {
      ...
      if (owner) {
        const ownerFiber = owner;
        invariant(ownerFiber.tag === ClassComponent);
...
function cloneChildFibers(current, workInProgress) {
  invariant(current === null || workInProgress.child === current.child);
...
function updateDeferredValue(value) {
  const [prevValue, setValue] = updateState();
  updateEffect(() => {
    const prevTransition = ReactCurrentBatchConfig$1.transition;
    ReactCurrentBatchConfig$1.transition = 1;
...
function rerenderDeferredValue(value) {
  const [prevValue, setValue] = rerenderState();
  updateEffect(() => {
    const prevTransition = ReactCurrentBatchConfig$1.transition;
    ReactCurrentBatchConfig$1.transition = 1;
...
function mountOpaqueIdentifier() {
  const makeId = makeClientIdInDEV.bind(
    null,
...
const { unstable_now: now$1 } = Scheduler;
let commitTime = 0;
  ...
  switch (workInProgress.tag) {
    ...
    case HostComponent: {
      ...
      if (current !== null && workInProgress.stateNode != null) {
        ...
        if (!newProps) {
          invariant(workInProgress.stateNode !== null); // This can happen when we abort work.
    ...
    case SuspenseComponent: {
...

      {
        ...
        if (suspenseState !== null && suspenseState.dehydrated !== null) {
          invariant(workInProgress.alternate !== null);
          resetHydrationState();
        }
      }
...
function commitWork(current, finishedWork) {
  switch (finishedWork.tag) {
    ...
    case HostText: {
      invariant(finishedWork.stateNode !== null);
...
let currentEventTransitionLane = NoLanes; // Dev only flag that tracks if passive effects are currently being flushed.
...
function performConcurrentWorkOnRoot(root, didTimeout) {
  ...
  invariant(); // Flush any pending passive effects before deciding which lanes to work on,
...
function completeUnitOfWork(unitOfWork) {
  ...
  do {
    ...
    if ((completedWork.flags & Incomplete) === NoFlags) {
      ...
      const next = unwindWork(completedWork, subtreeRenderLanes); // Because this fiber did not complete, don't reset its expiration time.
...
function commitRootImpl(root, renderPriorityLevel) {
  ...
  root.finishedWork = null;
  root.finishedLanes = NoLanes;
  invariant(finishedWork !== root.current); // commitRoot never returns a continuation; it always finishes synchronously.
...

{
  ...
  beginWork$1 = (current, unitOfWork, lanes) => {
...
const warnIfNotCurrentlyActingUpdatesInDev = warnIfNotCurrentlyActingUpdatesInDEV; // In tests, we want to enforce a mocked scheduler.
...
const findHostInstancesForRefresh = (root, families) => {
  {
...
function updateContainer(element, container, parentComponent, callback) {
  ...
  const root = scheduleUpdateOnFiber(current$1, lane, eventTime);
...

{
  ...
  const copyWithSetImpl = (obj, path, index, value) => {
    ...
    const key = path[index];
    const updated = Array.isArray(obj) ? obj.slice() : { ...obj }; // $FlowFixMe number or string is fine here
...
function unstable_renderSubtreeIntoContainer(
  ...
  invariant(isValidContainer(containerNode));
  invariant(parentComponent != null && has(parentComponent));
  return legacyRenderSubtreeIntoContainer(
    parentComponent,
    element,
    containerNode,
...
function createPortal$1(children, container, key = null) {
  invariant(isValidContainer(container)); // TODO: pass ReactDOM portal implementation as third argument
